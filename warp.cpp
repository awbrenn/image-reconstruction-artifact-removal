...
pixel ** OUTPUT_PIXMAP = NULL;
pixel ** FIXED_PIXMAP = NULL; // "fixed" is vague, but this is my artifact free pixmap
pixel ** UNFIXED_PIXMAP = NULL; // warped pixmap with artifacts
...
void populateOutputPixmap(pixel ** &input_pixmap, pixel ** &output_pixmap) {
    ...
    for (int row = 0; row < IMAGE_HEIGHT; row++)
        for (int col = 0; col < IMAGE_WIDTH; col++) {
            u = (int) ((double)IMAGE_WIDTH * (1.0 - sqrt(1.0-((double)col/(double)IMAGE_WIDTH))) * (1.6 + 0.5 * cos((2 * PI * (double) row)/(double)IMAGE_HEIGHT)));
            v = (int) ((double)IMAGE_HEIGHT * (1.0 - sqrt(1.0-((double)row/(double)IMAGE_HEIGHT))) * (1.6 + 0.5 * cos((2 * PI * (double) col)/(double)IMAGE_WIDTH)));
            // basic interpolation
...
void fixReconstructionArtifacts(pixel ** &input_pixmap, pixel ** &output_pixmap) {
    ...
    for (int row = 0; row < IMAGE_HEIGHT; row++)
        for (int col = 0; col < IMAGE_WIDTH; col++) {
            u = ((double)IMAGE_WIDTH * (1.0 - sqrt(1.0-((double)col/(double)IMAGE_WIDTH))) * (1.6 + 0.5 * cos((2 * PI * (double) row)/(double)IMAGE_HEIGHT)));
            v = ((double)IMAGE_HEIGHT * (1.0 - sqrt(1.0-((double)row/(double)IMAGE_HEIGHT))) * (1.6 + 0.5 * cos((2 * PI * (double) col)/(double)IMAGE_WIDTH)));
